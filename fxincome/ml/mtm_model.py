...
def generate_dataset(main_df, root_path, val_ratio=0.1, test_ratio=0.1):
    ...
    train_X = train_df[MTM_PARAM.TRAIN_FEATS]
    train_y = train_df[MTM_PARAM.LABELS].squeeze().to_numpy()
    val_X = val_df[MTM_PARAM.TRAIN_FEATS]
    val_y = val_df[MTM_PARAM.LABELS].squeeze().to_numpy()
    test_X = test_df[MTM_PARAM.TRAIN_FEATS]
    test_y = test_df[MTM_PARAM.LABELS].squeeze().to_numpy()
...
def xgb_train(train_X, train_y, val_X, val_y, test_X, test_y):
    ...
    epoch = test_score = train_score = val_score = obb_score = 0
    epoch_limit = 1000
    while epoch < epoch_limit and (test_score < 0.61 or train_score < 0.63 or val_score < 0.63):
        logger.info(f"Epoch {epoch}")
        xgb_search.fit(train_X, train_y, eval_set=[(val_X, val_y)], early_stopping_rounds=5, verbose=False)
...
def svm_poly_train(train_X, train_y, val_X, val_y, test_X, test_y):
    ...
    svm_search = RandomizedSearchCV(clf, param_distributions=param_dist, n_iter=100,
                                    n_jobs=-1, pre_dispatch=64)
    svm_search.fit(combined_train_X, combined_train_y)
    best_svm = svm_search.best_estimator_
    test_score, train_score, val_score, obb_score = report_model(best_svm,
