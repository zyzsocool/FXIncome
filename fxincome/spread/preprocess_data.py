...
def feature_engineering(leg1_code: str, leg2_code: str, days_back: int, n_samples: int,
                        days_forward: int, spread_threshold: float, spread_is_wider: bool) -> pd.DataFrame:
    """
    ...
    df['IPO_DATE_' + 'LEG2'] = df['IPO_DATE_' + 'LEG2'].dt.date
    df['DAYS_SINCE_LEG2_IPO'] = (df['DATE'] - df['IPO_DATE_' + 'LEG2']).dt.days
    df['SPREAD'] = df['YTM_' + 'LEG2'] - df['YTM_' + 'LEG1']
    df['VOL_DIFF'] = df['VOL_' + 'LEG2'] - df['VOL_' + 'LEG1']
    df['OUT_BAL_DIFF'] = df['OUT_BAL_' + 'LEG2'] - df['OUT_BAL_' + 'LEG1']
    for i in range(1, days_back + 1):
    ...
    for i in range(1, days_back + 1):
        df[f'VOL_LEG1_t-{i}'] = df['VOL_' + 'LEG1'].shift(i)
    for i in range(1, days_back + 1):
        df[f'VOL_LEG2_t-{i}'] = df['VOL_' + 'LEG2'].shift(i)
    for i in range(1, days_back + 1):
        df[f'YTM_LEG1_t-{i}'] = df['YTM_' + 'LEG1'].shift(i)
    for i in range(1, days_back + 1):
        df[f'YTM_LEG2_t-{i}'] = df['YTM_' + 'LEG2'].shift(i)
    for i in range(1, days_back + 1):
        df[f'OUT_BAL_LEG1_t-{i}'] = df['OUT_BAL_' + 'LEG1'].shift(i)
    for i in range(1, days_back + 1):
        df[f'OUT_BAL_LEG2_t-{i}'] = df['OUT_BAL_' + 'LEG2'].shift(i)

    first_date = df.iloc[0]['IPO_DATE_' + 'LEG2']  # leg2's IPO date, for selecting n samples.
...
feature_engineering('210215', '220205', days_back=5, n_samples=150, days_forward=10, spread_threshold=0.02,
                    spread_is_wider=True)
