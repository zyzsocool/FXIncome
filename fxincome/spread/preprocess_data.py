...
def download_data(
    start_date: datetime.date = datetime.date(2019, 1, 1),
    ...
    for code in SPREAD.CDB_CODES:
        ...
        df.loc[df.DATE >= df.IPO_DATE, "VOL"] = df.loc[
            df.DATE >= df.IPO_DATE, "VOL"
        ].fillna(0.0)
...
def feature_engineering(
    ...
    df_1 = pd.read_csv(input_file, parse_dates=["DATE", "IPO_DATE"]).drop(
        columns=["CODE"]
    )
    input_file = PATH.SPREAD_DATA + leg2_code + ".csv"
    df_2 = pd.read_csv(input_file, parse_dates=["DATE", "IPO_DATE"]).drop(
        columns=["CODE"]
    )
    df = pd.merge(df_1, df_2, on="DATE", how="inner", suffixes=("_LEG1", "_LEG2"))
    df.to_csv(
        PATH.SPREAD_DATA + f"{leg1_code}_{leg2_code}.csv", index=False, encoding="utf-8"
    )
    ...
    #  MACD of spread
    df["SPREAD_MACD"] = (
        df.SPREAD.ewm(span=days_back / 2, adjust=False).mean()
        - df.SPREAD.ewm(span=days_back, adjust=False).mean()
    )
    #  MACD of OUT_BAL_DIFF
    df["OUT_BAL_DIFF_MACD"] = (
        df.OUT_BAL_DIFF.ewm(span=days_back / 2, adjust=False).mean()
        - df.OUT_BAL_DIFF.ewm(span=days_back, adjust=False).mean()
    )
    ...
    df[f"VOL_DIFF_AVG_{days_back}"] = df.VOL_DIFF.rolling(
        days_back, closed="left"
    ).mean()
    #  Mean of past [t-1, t-2... t-days_back] days' outstanding balance difference
    df[f"OUT_BAL_DIFF_AVG_{days_back}"] = df.OUT_BAL_DIFF.rolling(
        days_back, closed="left"
    ).mean()
    ...
    first_date = df.iloc[0][
        "IPO_DATE_LEG2"
    ]  # leg2's IPO date, for selecting n samples.

    # Label Engineering
    df = df.sort_values(by="DATE")
    if spread_threshold >= 0:  # spread is wider
        df[f"max_in_future"] = df.SPREAD.rolling(
            pd.api.indexers.FixedForwardWindowIndexer(window_size=days_forward + 1)
        ).max()
        df["LABEL"] = df.apply(
            lambda row: 1
            if (row.max_in_future - row.SPREAD) >= spread_threshold
            else 0,
            axis=1,
        )
        ...
        df["LABEL"] = df.apply(
            lambda row: 1
            if (row.min_in_future - row.SPREAD) <= spread_threshold
            else 0,
            axis=1,
        )
    ...
    logger.info(
        f"Label 1 ratio: {df.LABEL.sum() / len(df):.2f}. Total samples: {len(df)}"
    )
    df = df[["DATE"] + features + ["LABEL"]]
    df.to_csv(
        PATH.SPREAD_DATA + f"{leg1_code}_{leg2_code}_FE.csv",
        index=False,
        encoding="utf-8",
    )
    if not keep_date:
...
def select_features(days_back: int) -> list[str]:
    ...
    def dynamic_feature_names(
        f_name: str, days_back: int = 0, avg: bool = False
    ) -> list[str]:
        """
...
def prepare_backtest_data(leg1_code: str, leg2_code: str):
    ...
    def get_bond(leg_code: str):
        error_code, rows = w.wss(
            leg_code + ".IB",
            "carrydate,maturitydate,couponrate,interestfrequency",
            usedf=True,
        )
        row = rows.iloc[0]
        issue_date = Date(
            row["CARRYDATE"].day, row["CARRYDATE"].month, row["CARRYDATE"].year
        )
        maturity_date = Date(
            row["MATURITYDATE"].day, row["MATURITYDATE"].month, row["MATURITYDATE"].year
        )
    ...
    for i in range(0, len(df)):
        ...
        df.loc[i, "ESTIMATED_PRICE1"] = bond1.full_price_from_ytm(
            settlement_date, df.loc[i, "YTM_LEG1"] / 100, YTMCalcType.CFETS
        )
        df.loc[i, "ESTIMATED_PRICE2"] = bond2.full_price_from_ytm(
            settlement_date, df.loc[i, "YTM_LEG2"] / 100, YTMCalcType.CFETS
        )
    ...
    if sum(max_out_bal_df.SPREAD.ge(0)) > 0:
        first_positive_spread_index = max_out_bal_df.SPREAD.ge(0).idxmax()
        df = df.loc[: first_positive_spread_index + 1]
    df = df.drop(["ESTIMATED_PRICE1", "ESTIMATED_PRICE2"], axis=1)
    df.to_csv(
        PATH.SPREAD_DATA + leg1_code + "_" + leg2_code + "_bt.csv",
        index=False,
        encoding="utf-8",
    )


download_data(start_date=datetime.date(2019, 1, 1), end_date=datetime.date(2023, 7, 25))
for i in range(0, 14):
