...
if __name__ == "__main__":
    ...
    asset_df = asset_df[
        (asset_df["bond_type"].isin(["政策银行债", "国债", "地方政府债"]))
    ...
    asset_df["period"] = asset_df["end_date"].apply(
        lambda x: round((x - date).days / 365)
    )
    asset_df["period2"] = asset_df["end_date"].apply(
        lambda x: round((x - date).days / 365, 2)
    )
    ...
    asset_df["ranking"] = (
        asset_df[["trading", "period"]]
        .groupby("period")
        .transform(lambda x: x >= maxx(x, activity_rank))
    )
    asset_df = asset_df[
        (asset_df["ranking"] > 0) & (asset_df["trading"] >= activity_trading)
    ...
    #  按照日期区间筛选展示债券
    asset_df = asset_df[
        (asset_df["end_date"] >= min_date) & (asset_df["end_date"] <= max_date)
    ]
    ...
    for i, j in asset_df.iterrows():
        ...
        for k in period_list:
            if k[1] <= j["end_date"]:
                ...
                asset_df.loc[i, k[0]] = bond_i.get_profit(
                    date, k[1], j["ytm"], end_ytm
                )[1]

    asset_df["bond"] = asset_df.apply(
        lambda x: "{}[{}Y][{:.2f}%]".format(x["code"], x["period2"], x["ytm"]), axis=1
    )
    ...
    reuslt_overview = reuslt_overview.applymap(
    ...
    for k in period_list:
        ...
        reuslt_k["bond_base_ytm"] = reuslt_k["bond_base"].apply(
            lambda x: asset_df.loc[x, "ytm"]
        )
        reuslt_k["bond_base_yeild"] = reuslt_k["bond_base"].apply(
            lambda x: asset_df.loc[x, k[0]]
        )
        reuslt_k["bond_base_period"] = reuslt_k["bond_base"].apply(
            lambda x: asset_df.loc[x, "period2"]
        )

        reuslt_k["bond_target_ytm"] = reuslt_k["bond_target"].apply(
            lambda x: asset_df.loc[x, "ytm"]
        )
        reuslt_k["bond_target_yeild"] = reuslt_k["bond_target"].apply(
            lambda x: asset_df.loc[x, k[0]]
        )
        reuslt_k["bond_target_period"] = reuslt_k["bond_target"].apply(
            lambda x: asset_df.loc[x, "period2"]
        )
        ...
        for i, j in reuslt_k.iterrows():
            ...
            if j["bond_base"] == j["bond_target"]:
                ...
                while True:
                    ...
                    # if i==18:
                    #     print(y)
                    yld = asset_dic[j["bond_base"]].get_profit(
                        date, k[1], j["bond_base_ytm"], y
                    )[1]
                    if abs(yld - j["bond_target_yeild"]) < 0.01:
                        break
                    if (abs(yld - j["bond_target_yeild"]) < 0.1) & (
                        abs(y - y_last) < 0.0001
                    ):
                        break
                ...
                reuslt_k.loc[i, "bp"] = (
                    y
                    - curve(
                        (asset_df.loc[j["bond_base"], "end_date"] - k[1]).days / 365
                    )
                ) * 100
        ...
        reuslt_k["target_bond"] = reuslt_k.apply(
            lambda x: "{}\n[{}Y]\n[{:.2f}%]\n[{:.2f}%]".format(
                x["bond_target"],
                x["bond_target_period"],
                x["bond_target_ytm"],
                x["bond_target_yeild"],
            ),
            axis=1,
        )
        rank_type = CategoricalDtype(
            list(reuslt_k["base_bond"].drop_duplicates()), ordered=True
        )

        reuslt_k["base_bond"] = reuslt_k["base_bond"].astype(rank_type)
        reuslt_k["target_bond"] = reuslt_k["target_bond"].astype(rank_type)
        result_k_pivot = pd.pivot_table(
            reuslt_k,
            index="base_bond",
            columns="target_bond",
            values="bp",
